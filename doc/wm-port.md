...
apps/wm/
  ...
  wm.l        the driver: wire the config's hooks into the event loop.
  config.l    THE PORTED xmonad.hs, in the DSL. This is what the user edits.
  host/wm.c   the connectu nif (already landed).
```
...
host-only (it speaks sockets), smoke-tested the `boot/wm.l` way (in `test_hostnif`) plus the
...
| `startupHook` | `startup-hook (>> ...)` | 1 | spawn + set-wm-name + set-cursor |
...
9. **Graduate** — `make install` a `wm` binary; restart-in-place (`sup+shift+o`-style) via the
