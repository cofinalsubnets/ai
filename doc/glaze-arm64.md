...
`crew/asm/x64.l`'s abstract file is deliberately tuned to SysV: g/Ip/Hp/Sp = abstract **r6/r5/r2/r1**,
acc = r0. `crew/asm/arm64.l` is an **identity** file (rN→xN), so the *same abstract numbers* would land
...
- **Assembler — done & gated** (`crew/asm/asmtest.l`, both targets, every encoding objdump/llvm-mc-verified):
...
- **The codegen retarget — three lanes done & full-`make test`-green**, in `emit.l`, ADDITIVE (the live
  ...
  - **group** `jitgroupir` (mutual recursion): `cggir`/`gargsir`/`mkhir`/`mkouterir`. Calls = `(call
    <name>)`+`(label <name>)` (crew/asm/ resolves rel offsets — the byte path's two-pass offset math is
...
- **Remaining (in order):**
  ...
  4. **special ops** — `peep`/`tally` (string), `mpeep`/`mpin` (map — already neutral IR via the
     `crew/asm/` probe, just wire it in), `pin` (cask), `cap`/`cup`/`two?` (chain). Needs byte load/store
