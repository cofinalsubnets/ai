...
lvm_t lvm_kcall,
 ...
 lvm_pin, lvm_peep, lvm_fputx, lvm_buf, lvm_bufnew, lvm_bcopy, lvm_call, lvm_call2, lvm_amap, lvm_amap2, lvm_areduce, lvm_armap, lvm_armapz, lvm_arreduce, lvm_rfoldw, lvm_acmap, lvm_acreduce, lvm_cfoldw, lvm_respec, lvm_fbits, lvm_forge,
...
lvm(lvm_vbin, int);
lvm(lvm_vmap1, g_flo_t (*)(g_flo_t));
lvm(lvm_vmap2, g_flo_t (*)(g_flo_t, g_flo_t));
...
// Complex-array elementwise lane (g_C): lvm_vbin diverts here when an operand is
...
lvm(lvm_cbin, int);
// Object-array elementwise lane (g_O): the broadcast engine lvm_vbin diverts
...
#define forget() (g_core_of(g)->root=(mm0),g)
...
#define nifs(_) \
 ...
 _(nif_rfoldw, "rfoldw", s3(lvm_rfoldw)) _(nif_cfoldw, "cfoldw", s3(lvm_cfoldw))\
...
static struct g *c0(struct g *g, lvm_t *y);
...
static g_inline struct g *c0_ix(struct g *g, struct env **c, lvm_t *i, word x) {
 return incl(*c, 2), g_push(g, 3, c1_ix, i, x); }

static g_inline struct g *c0_i(struct g *g, struct env **c, lvm_t *i) {
...
static g_noinline struct g *c0(struct g *g, lvm_t *y) {
...
cata1(c1_cond_push_branch, g = gxl(g_push(g, 2, Kp, (*c)->branches)), (*c)->branches = g_ok(g) ? pop1(g) : nil)
cata1(c1_cond_push_exit, g = gxl(g_push(g, 2, Kp, (*c)->exits)), (*c)->exits = g_ok(g) ? pop1(g) : nil)
...
static g_inline Ana(ana_q) { return c0_ix(g, c, lvm_quote, x); }
...
static g_inline struct g *ana_d(struct g *g, struct env **b, word exp) {
 ...
 struct g_r *mm0 = g_core_of(g)->root;
 mm(g, &exp);
...
// lvm_yield_sw_mono can't call g_wait_fds directly with a stack pointer
...
lvm(lvm_cond) { return Ip = g_nilp(*Sp++) ? Ip[1].m : Ip + 2, Continue(); }
lvm(lvm_unc) {
 Have1();
 *--Sp = Ip[1].x;
 Ip = Ip[2].m;
...
// a buf (byte-identical body, kept distinct by g_noicf -- see lvm_buf).
lvm(lvm_port_io) {
  Ip = cell(*++Sp);
  *Sp = putfix(1);
  return Continue(); }
...
// --- one non-recursive STRUCTURAL reader for g_read1 (multi=0) and g_reads
...
static struct g *gzread1op(struct g *g, int c, int *pending) {
...
static struct g *gz_parse(struct g *g, bool multi) {
 ...
 for (;;) {
  ...
  switch (c) {
   ...
   case '#': g = push_wrap(g, "hash"); continue;       // (#! comments die in g_z_getc)
...
// sys
// ============================================================================
op11(lvm_clock, putfix(g_clock() - getfix(Sp[0])))
...
// g_mapdel: delete k, backward-shift the probe chain so no tombstone is
...
// (amap code in out) — the array kernel: fill z-array `out` cell-by-cell with
// fn(in_cell), fn = the native code in buf `code`. z-array cells are RAW int64
// and z-array arithmetic WRAPS, so a raw-in/raw-out wrapping kernel matches love
// exactly (no tag, no overflow guard). in/out must be equal-length g_Z arrays.
// The tight loop is in C (it knows the tuple layout and never allocates, so no GC
// moves the cell buffers mid-loop); the per-element arithmetic is native. Returns
// out. Arity 3.
lvm(lvm_amap) {
 word code = Sp[0], in = Sp[1], out = Sp[2];
 if (bufp(code) && arrp(in) && arrp(out)
     && tuple(in)->type == g_Z && tuple(out)->type == g_Z
     && tuple_nelem(tuple(in)) == tuple_nelem(tuple(out))) {
  g_word (*fn)(g_word) = (g_word (*)(g_word)) txt(buf_str(code));
  intptr_t *ic = (intptr_t*) tuple_data(tuple(in)),
...
// (areduce code init in) — fold the native 2-arg kernel over z-array `in`:
// acc = init; for each cell, acc = fn(acc, cell); return acc. fn takes the
// accumulator in %rdi and the cell in %rsi (raw int64). The accumulator wraps at
// int64 like z-array arithmetic, and the result boxes via emit_int -- a fixnum if
// it fits, else a rank-0 wide scalar -- exactly like asum/aprod. init is a fixnum.
// Validity check FIRST, then Have (so a GC retry re-runs the whole nif, not just
// the box), then the no-allocation loop. Arity 3.
lvm(lvm_areduce) {
 if (!(bufp(Sp[0]) && fixp(Sp[1]) && arrp(Sp[2]) && tuple(Sp[2])->type == g_Z))
  return Sp[2] = Sp[1], Sp += 2, Ip++, Continue();   // not applicable -> init
 Have(box_req);
 word code = Sp[0], in = Sp[2];
 g_word (*fn)(g_word, g_word) = (g_word (*)(g_word, g_word)) txt(buf_str(code));
 intptr_t *ic = (intptr_t*) tuple_data(tuple(in)), acc = getfix(Sp[1]);
...
// (armapz code in out) — float-array COMPARISON kernel: r-array `in` (f64 cells)
// -> z-array `out` (int cells), fn a g_flo_t(*)(g_word) -- wait, double in %xmm0,
// INT out in %rax. A float comparison yields a z mask (0/1), like love's `<`/`=`
// on an r-array, so the output array is g_Z while the input is g_R.
lvm(lvm_armapz) {
 word code = Sp[0], in = Sp[1], out = Sp[2];
 if (bufp(code) && arrp(in) && arrp(out)
     && tuple(in)->type == g_R && tuple(out)->type == g_Z
     && tuple_nelem(tuple(in)) == tuple_nelem(tuple(out))) {
  intptr_t (*fn)(g_flo_t) = (intptr_t (*)(g_flo_t)) txt(buf_str(code));  // double -> int
  g_flo_t *ic = (g_flo_t*) tuple_data(tuple(in));
  intptr_t *oc = (intptr_t*) tuple_data(tuple(out));
  for (uintptr_t i = 0, n = tuple_nelem(tuple(in)); i < n; i++) oc[i] = fn(ic[i]);
 }
 return Sp[2] = out, Sp += 2, Ip++, Continue(); }   // arity 3: collapse two, out at the new top
...
// (rfoldw code init in) — WHOLE-LOOP float fold: the forged code owns the loop
// itself, so the op is inlined and there is NO per-cell call (cf. arreduce, which
// calls fn per cell). fn(g_flo_t *cells, uintptr_t n, g_flo_t init) -> g_flo_t
// (cells %rdi, n %rsi, init %xmm0, result %xmm0). One C->native call total.
lvm(lvm_rfoldw) {
 if (!(bufp(Sp[0]) && (fixp(Sp[1]) || flop(Sp[1]) || widep(Sp[1]) || bigp(Sp[1]))
       && arrp(Sp[2]) && tuple(Sp[2])->type == g_R))
  return Sp[2] = Sp[1], Sp += 2, Ip++, Continue();
 Have(box_req);
 g_flo_t (*fn)(g_flo_t*, uintptr_t, g_flo_t) =
   (g_flo_t (*)(g_flo_t*, uintptr_t, g_flo_t)) txt(buf_str(Sp[0]));
 g_flo_t acc = fn((g_flo_t*) tuple_data(tuple(Sp[2])), tuple_nelem(tuple(Sp[2])), toflo(Sp[1]));
 word _res; emit_flo(acc);
 return Sp[2] = _res, Sp += 2, Ip++, Continue(); }   // arity 3
...
// (cfoldw code init in) — WHOLE-LOOP complex fold: forged code owns the loop, op
// inlined, no per-cell call (cf. acreduce). fn(double *cells, uintptr_t n,
// struct g_c2 init) -> struct g_c2 (cells %rdi, n %rsi, init %xmm0/%xmm1,
// result %xmm0/%xmm1). Cells are interleaved (re,im); the loop strides by 2.
lvm(lvm_cfoldw) {
 if (!(bufp(Sp[0]) && Cp(Sp[1]) && arrp(Sp[2]) && tuple(Sp[2])->type == g_C))
  return Sp[2] = Sp[1], Sp += 2, Ip++, Continue();
 Have(cplx_req);
 struct g_c2 (*fn)(double*, uintptr_t, struct g_c2) =
   (struct g_c2 (*)(double*, uintptr_t, struct g_c2)) txt(buf_str(Sp[0]));
 struct g_c2 acc = fn((double*) tuple_data(tuple(Sp[2])), tuple_nelem(tuple(Sp[2])),
                      (struct g_c2){ cplx_re(Sp[1]), cplx_im(Sp[1]) });
 struct g_tuple *bx = ini_scalar((struct g_tuple*) Hp, g_C); Hp += cplx_req;
 cplx_put(bx, acc.re, acc.im);
 return Sp[2] = word(bx), Sp += 2, Ip++, Continue(); }   // arity 3
...
// compiled body. The JIT uses this to give a native closure the IDENTITY of the
// source lambda it replaces: eqv is fn_src-based (α-compares the stashed \-expr,
// and the 1/0 numeral bridges read it too), so a native closure that carries the
// original source and holds NO captures (its forged buf is a folded constant, not
// a curried arg -> not fn_partialp) compares EXACTLY like the interpreted closure.
// No-op unless lam is a non-partial, leading-cell lambda and src is a pair.
lvm(lvm_respec) {
 word lam = Sp[0], src = Sp[1];
 struct g *c = g_core_of(g);
 if (lamp(lam) && !datp(lam) && !fn_partialp(cell(lam)) && twop(src)
     && ptr(lam) > ptr(c) && ptr(lam) < ptr(c) + c->len
...
#define bit_slow(n, c_op) static lvm(lvm_##n##_slow) {               \
 ...
 return *++Sp = _res, Ip++, Continue(); }
#define mvm1(n) lvm(lvm_##n) { return Ap(lvm_math1, g, g_##n); }
...
static lvm(lvm_math2, g_flo_t (*fn)(g_flo_t, g_flo_t)) {
 word a = Sp[0], b = Sp[1];
...
lvm(lvm_log) {
 ...
 else return Ap(lvm_math1, g, g_log);
 Have(cplx_req);
...
// Pack(g); g = g_big_binop(g, vop); Unpack(g); Continue();  (cf. lvm_gc).
...
static struct g *g_bmul_setup(struct g *g) {
 ...
 ((struct g_buf*) k)->ap = lvm_buf, ((struct g_buf*) k)->str = s, tagtext(k, Width(struct g_buf));
...
lvm(lvm_arr) {
 ...
 if (rank > maxrank || (ty == g_O && rank == 0)) return Sp[2] = nil, Sp += 2, Ip++, Continue();
...
lvm(lvm_vmap1, g_flo_t (*fn)(g_flo_t)) {
...
// divisibility). g_noinline: its &-taken stride/odometer arrays stay off lvm_vbin's
...
lvm(lvm_vmap2, g_flo_t (*fn)(g_flo_t, g_flo_t)) {
 word a = Sp[0], b = Sp[1];
 bool aarr = arrp(a), barr = arrp(b);
 if (!(aarr || isnum(a)) || !(barr || isnum(b)))   // each operand: array or scalar
  return *++Sp = nil, Ip++, Continue();
...
// Pack'd body of lvm_obin (operands at g->sp[0..1], >=1 is a g_O array).
...
// g_noinline helper so the lvm wrapper keeps its trailing tail call; no
...
// real pow. &-locals stay in this g_noinline helper, off lvm_pow's tail call.
...
// odometer arrays live in this g_noinline fill so lvm_cplx keeps its tail call.
...
// array, same shape). &-free, but g_noinline to keep lvm_carg's tail call clean.
