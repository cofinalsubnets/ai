(: egg '(
  ...
  ; line editor: a zipper (table) with 'l (chars left of the cursor,
  ; reversed) and 'r (focus and rest, in order); 'eof latches ^D. the
  ; cursor never crosses a newline, so a redraw only ever touches the
  ; current physical line. used by the repl.
