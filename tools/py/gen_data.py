...
ai_data.NN input section -- before the link, no link needed. This script
...
def fail(msg):
    sys.stderr.write("gen_data: error: %s\n" % msg)
    sys.exit(1)


def warn(msg):
    sys.stderr.write("gen_data: warning: %s\n" % msg)
...
def stride_from(path):
    ...
    The sentinels share one body, so every ai_data.NN section has the same
    ...
    secs.sort(key=lambda s: s[0])   # ai_data.00 .. .04, enum order
    if len(secs) != N:
...
#ifndef _g_data_h
#define _g_data_h
// GENERATED by tools/gen_data.py from data.o -- do not edit by hand.
//
...
static ai_inline enum q ai_typ(union u *o) {
 static const enum q kinds[] = { KVec, KBig, KString, KSym, KChain, KFlo, KWide, KCplx };
...
def main():
    ...
    if "-o" in args:
        ...
        del args[i:i + 2]
    if len(args) != 1:
        fail("usage: gen_data.py <data.o> [-o data.h]")
