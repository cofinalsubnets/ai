...
case $lang in
  # ai: for `float` and `fib`, load the native glaze (ai/glaze/emit.l + auto.l) AHEAD of the bench on
  # an x86-64 host -- the recognizers then read the bench's UNCHANGED source and compile it to native
  # (float.l's float-recurrence grid -> a whole-grid SSE kernel; fib.l's recursive arithmetic -> a
  # native call tree), the ai analogue of LuaJIT auto-JITting Lua. The glaze self-tests print to
  # stderr (discarded here); other benches stay interpreted (the glaze only matches those shapes).
  ai)            ext=l;    bin=../out/host/ai;  cmd='cat $({ [ "$b" = float ] || [ "$b" = fib ]; } && [ "$(uname -m)" = x86_64 ] && printf "%s %s " ../ai/glaze/emit.l ../ai/glaze/auto.l) bench.l benches/$b.l | ../out/host/ai' ;;
