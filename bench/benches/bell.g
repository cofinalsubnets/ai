...
(: (bell-run limit)
    (: miss  (nom 0)  facts (hashn 0)  bells (hashn 0)
       digits "0123456789abcdefghijklmnopqrstuvwxyz"  base (pin digits)
       ...
       (show n) (showloop n "")
       (gen i acc) (: b (show (bell i)) (? (<= (pin b) limit) (gen (+ i 1) (+ acc (pin b))) acc))
     (gen 0 0))
