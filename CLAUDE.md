...
; the core laws are ALSO machine-checked axiom-free in proof/rocq/spec.v (wired into `make test_all` as
...
;   gc/glaze/sat/asm/wm, tool diffs, arm64 + the qemu kernel + wasm; `make valg` for memory. one file: `out/host/ai test/x.l` -- but the corpus runs CONCATENATED in one
...
;   host/pty.c), cook (make-in-ai, apps/cook/cook.l), wm (the X11 window manager, apps/wm/), sat (the CDCL
;   solver, apps/sat/), inle (freestanding agent-kernel, port/inle/), quay (the terminal
;   driver, port/quay/ + apps/quay/ink.l the screensaver -- host-only, probe under a pty). apps
...
; * MODULE BOOKS: a baked service sweeps its leaked names into ONE public book -- apps/asm/export.l and
...
; f at zero operands -- so (:) (?) (\) all read (), and a nullary HELPER call is the same law and
; the same trap: `(go)` is `go` UNRUN (the ⚠ bullet up top -- fire thunks with the unit, `(go ())`).
; demo:
...
; net's order-signed magnitude, max(0, ceil) capped at the greatest charm, with the invariant
; !x == (0 = $x). `$` is the SOLE
...
; as the unit when chained through ANY arithmetic lane, not annihilates: `(* (* 'a 'b) 5)` is 5,
; `(- ('a - 'b) 5)` too -- the unit rides through - / // % and the bitwise ops, either side). `/` is *true* division: an inexact
; integer quotient promotes to float ((/ 1 2) is 0.5), an exact one stays integer ((/ 4 2) is 2); 1/0
; gives IEEE infinity (ieee-inf, a float token), 0/0 gives 0.0 -- NaN COLLAPSES to the zero gem, so
...
1 / 0                ; ieee-inf
(/ 0 0)              ; 0.0     NaN collapses to the zero gem
8 | 4 | 2 | 1        ; 15      bitwise
...
; stays false, a number vs a closure included: numerals ACT as their lambdas under apply ((1 x) = x,
; (0 x) = 1), but `=` never says so (bridging them broke congruence, the order, and transitivity).
; demo:
(\ x x) = (\ y y)            ; true   alpha: bound vars by position
(\ x (+ x 1)) = (\ x (+ x 1)); true   ... yet distinct objects (id? false)
(\ x x) = (\ y z)            ; false  free /= bound
!(1 = (\ x x)) !(0 = (\ _ 1)) ; true  no numeral bridge: representation-crossing stays false
...
; (0 annihilates *, () does not). and the unit RIDES THROUGH the rest of the arithmetic ops the same
; way -- - / // % & | ^ << >>, either side: `5 - ()` AND `() - 5` are 5 (no 0-face projection; the op
; never happens), so an undefined op's () vanishes through EVERY arithmetic chain, not just + and *.
; the zero point counts, as does any fresh mint; a NAMED symbol still
...
5 - ()               ; 5         the unit rides through EVERY arithmetic lane (() - 5 is 5 too: no 0-face)
...
(3 3 3)              ; 7625597484987
i * i                ; ~(-1.0 0.0)  = -1, the algebraic heart of euler
(log -1) = i * pi    ; true      euler, in the EXACT direction
((/ 1 2) -1)         ; ~(0.0 1.0)   sqrt of -1: i, principal, exact
...
i                    ; ~(0.0 1.0)   i = ~(0 1)
~(1 2) * ~(3 4)      ; ~(-5.0 10.0)
(conj ~(2 3))        ; ~(2.0 -3.0)  ~ is conjugation, an involution
(re ~(2 3))          ; 2.0
(net (array 2 ~(1 2) ~(3 4)))   ; ~(4.0 6.0)   complex arrays fold complex
...
; l) orders by the total order, in C (descending = rev); (sortby le l) takes a predicate. (jot x)
; counts out the first x charms '(0 .. x-1) -- a range, the section of tally ((tally (jot n)) = n).
...
; `(string nom)` reads the name back. `()` reads as the zero point ITSELF, distinct from 0 (the
; nil-ontology work flips lingering 0-for-nothing cases to (), never the reverse); (intern "") is ()
; (the empty symbol is the +/* unit).
...
; datum BOXES it: #x = #(0 x), a fresh mutable hash pinning x at 0 (a 1-entry box, truthy). #()
; rides the literal lane instead -- zero pairs, the EMPTY tablet, falsy (() is distinct from 0:
; there is no box of nothing). accessors are
...
; demo:
(book? #())                   ; true   yet #() is the EMPTY tablet, not #0 (!!#() ; 0)
...
; GLOBAL (rand n)/(randf n) ride a hidden clock-seeded stream (zero setup; (rng-set (seed n)) pins it
; for a reproducible run, rng-get reads it). C holds no state: only seed + the pure steps under
; random; rand/randf are prel lisp. a global `help` receives every raise as (help s a b): s = the status
...
; it prints the always-on `;; missing nom` face and reads the zero point. the read happens where the code says: a closure captures its free globals
...
not-in-the-book      ; ()        a missing nom reads the zero point (helpless; the `;; missing` face prints)
